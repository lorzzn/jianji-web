...
const ZPostCard:FC<ZPostCardProps> = ({ post }) => {
  ...
  return <div className={twclx([
    ...
    <div className={twclx([
      ...
      {/* 标签 */}
      {
        !isEmpty(post.tags) && <div className="pt-6 relative space-x-2">
          <RiHashtag className="absolute my-1 -left-4" size={"1rem"} />
          {
            post.tags?.map(tag => {
              return <HoverFloating key={tag.value} className="rounded-full bg-gray-800 text-white px-2 py-0.5 text-xs inline-block select-none" >
                {tag.label}
              </HoverFloating>
            })
          }
