...
export interface ZModalBoxButton
...
const ZModalBox = forwardRef<ZModalRef, ZModalBoxProps>((props, ref) => {
  ...
  return (
    <ZModal
      ...
      title={props.title}
      classNames={{
        modal: twclx([" min-h-unset rounded-lg min-w-96"]),
        footer: twclx([" flex flex-row justify-end space-x-2"]),
      }}
      footer={<>{props.buttons?.map((button) => <ZButton {...button}>{button.children}</ZButton>)}</>}
    >
