  ...
  return (
    <div
      className={classNames([
        "flex items-center leading-10 text-gray-800",
        { "border-b pb-1 mb-1 border-gray-300": border },
      ])}
    >
...
const UserDialog: FC = () => {
  ...
  const settingsPageContentRender = (item: INavItem | undefined) => {
    return (
      <div className="flex flex-col justify-between h-full overflow-hidden ml-3">
        {item?.value === 0 && (
          <div className="w-full h-full relative">
            <motion.div
              ...
              variants={{
                show: { scale: 1, left: 0 },
                hide: { scale: 0.9, left: "-10%" },
              }}
            >
              <ZImage src={userInfo.avatar + "&s=192"} className="w-24 h-24 rounded-full" />

              <div className="mt-6 mb-6">{userInfo.name}</div>
            ...
            >
              ...
              <div className="px-6 mt-3">
                ...
                <div className="text-sm mt-3 mb-3">
                  <span>前往</span>
                  <a className="text-blue-500" href="https://cravatar.com/" target="_blank" rel="noopener noreferrer">
                    {" "}
                    Cravatar 初认头像{" "}
                  </a>
                  <span>修改</span>
                ...
                <ZForm className="flex flex-col h-full justify-between">
                  <div>
        ...
        {item?.value === 0 && (
          <div className="flex items-center justify-center">
            <ZButton
              scale={"large"}
              className="w-48"
              onClick={isEdit ? handleSubmit(submitNewUserInfo) : onEditUserInfoClick}
              loading={saveLoading}
            >
              {isEdit ? "保存" : "编辑资料"}
            </ZButton>
